...
- (void)viewDidAppear:(BOOL)animated{
	...
	//Make this object the delegate for location_manager
	...
	[self presentModalViewController:self.my_ipc animated:NO];
*/
...
- (IBAction)start_recording:(id)sender  // main start button
{
	...
	else	// it is off and should start recording
	{
		...
		NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"APMS" withExtension:@"momd"];
		managedObjectModel_ = [[NSManagedObjectModel alloc] initWithContentsOfURL:modelURL]; 
		
		
		NSString *stringFromDate2 = [NSString stringWithFormat:@"%@.sqlite", string_From_Date]; // naming the file (date).sqlite
		...
		// start the updates
		[motion_manager	setAccelerometerUpdateInterval:0.2];
		[motion_manager startAccelerometerUpdates];
		
		[motion_manager	setGyroUpdateInterval:0.2];
...
{
	if (!motion_manager.accelerometerActive) {
		// start the updates
		[motion_manager	setAccelerometerUpdateInterval:0.2];
		[motion_manager startAccelerometerUpdates];
	...
	NSLog(@"It has been a time interval");
	...
	if ((fixed_seconds-secondselapsed) <= 0.2) 
	{
		...
		self.oldtime = location_manager.location.timestamp;	//set old time to this time
		NSLog(@"%@", oldtime);
		...
		[formatter_ setDateFormat:@"MM/dd/yyyy"];
		[formatter_ setTimeZone:[NSTimeZone timeZoneWithName:@"PST"]];
		stringFromDate = [formatter_ stringFromDate:timestamp_date];
		[formatter_ release];
			
		xmlString = [NSString stringWithFormat:@"<?xml version=\"1.0\"?>\n<table>user01\n<passwordin>ta6aSU3a</passwordin>", self.string_From_Date];	
			
		xmlString = [xmlString stringByAppendingFormat:@"\n<date>%@</date>", [NSString stringWithFormat:@"%@", stringFromDate]];
		...
		xmlString = [xmlString stringByAppendingFormat:@"\n<time>%i</time>", [NSString stringWithFormat:@"%@", stringFromDate]]; 
		xmlString = [xmlString stringByAppendingFormat:@"\n<latt>%@</latt>", [NSNumber numberWithFloat:location2d.latitude]];		 
		xmlString = [xmlString stringByAppendingFormat:@"\n<long>%@</long>", [NSNumber numberWithFloat:location2d.longitude]];		
		xmlString = [xmlString stringByAppendingFormat:@"\n<hor_acc>%@</hor_acc>", [NSNumber numberWithFloat:location_manager.location.horizontalAccuracy ]];
		
		xmlString = [xmlString stringByAppendingFormat:@"\n<dist_int>%@</dist_int>", [NSNumber numberWithFloat:self.dist]];
		xmlString = [xmlString stringByAppendingFormat:@"\n<tot_dist>%@</tot_dist>", [NSNumber numberWithFloat:(self.running_total/1000)]];
		NSLog(@"Mistake after this");

		xmlString = [xmlString stringByAppendingFormat:@"\n<speed>%@</speed>", [NSNumber numberWithFloat:location_manager.location.speed]];
		xmlString = [xmlString stringByAppendingFormat:@"\n<heading>%@</heading>", [NSNumber numberWithFloat:location_manager.location.course]];
		

			
			
		xmlString = [xmlString stringByAppendingFormat:@"\n<acc_x>%@</acc_x>", [NSNumber numberWithFloat:acc.x]]; 
		xmlString = [xmlString stringByAppendingFormat:@"\n<acc_y>%@</acc_y>", [NSNumber numberWithFloat:acc.y]]; 
		xmlString = [xmlString stringByAppendingFormat:@"\n<acc_z>%@</acc_z>", [NSNumber numberWithFloat:acc.z]];
			
		xmlString = [xmlString stringByAppendingFormat:@"\n<gyro_x>%@</gyro_x>", [NSNumber numberWithFloat:rotate.x]]; 
		xmlString = [xmlString stringByAppendingFormat:@"\n<gyro_y>%@</gyro_y>", [NSNumber numberWithFloat:rotate.y]];
		xmlString = [xmlString stringByAppendingFormat:@"\n<gyro_z>%@</gyro_z>\n<passwordout>sqwb3QQs</passwordout>\n</table>", [NSNumber numberWithFloat:rotate.z]];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
	if (buttonIndex == 1) {						// if click came from YES
		...
		while (i < [timestamps_array count]) {
			
			NSArray *contents = [fileManager contentsOfDirectoryAtPath:[timestamps_array objectAtIndex:i] error:nil];
			
			for (NSString *file in contents) {
				BOOL success = [fileManager removeItemAtPath:[[timestamps_array objectAtIndex:i] stringByAppendingPathComponent:file] error:&error];
				
				if(!success){
					NSLog(@"couldn't delete file: %@",error);
				}
			}
			[fileManager removeItemAtPath:[timestamps_array objectAtIndex:i] error:&error];
			i++;
		}
		*/
